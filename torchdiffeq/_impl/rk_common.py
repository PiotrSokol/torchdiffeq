...
def _runge_kutta_step(func, y0, f0, t0, dt, tableau):
    ...
    k = tuple(map(lambda x: [x], f0)) # puts state from a tuple into a tuple containing a list
    for alpha_i, beta_i in zip(tableau.alpha, tableau.beta):  #b_i is the lower diagonal row vector of the RK matrix
        # b is typically denoted by A
        # c_i is the i^th  stage weight typically denoted by b
        # alpha_i is the  internal stage weight node typically denoted by c
        ti = t0 + alpha_i * dt
